...
By default, website capabilities are **passive**: they fetch the front page, its plain-http variant, `robots.txt`, `/.well-known/security.txt`, and **at most 5 same-site pages linked from the front page**, one at a time, plus a HEAD request (headers only) for up to 15 more of those links to confirm they work. `robots.txt` is honoured, and login, logout, admin, cart and file links are never followed, the way a careful reader would browse.

**JavaScript-built pages** (React, Next.js, Vue, site builders), where the HTML alone contains fewer than 150 words, are rendered in headless Chromium when it is installed. Chromium gets no network of its own: every request the page makes (its scripts, and API calls, including to third-party CDNs) is performed by the scanner's guarded client. That client still blocks private networks in hosted mode and records every host in the report. Images, fonts, media and known analytics or tracking services are never requested, so a scan does not count as a visit in the site's analytics. Use `--no-render` to disable rendering. Product Completeness makes one more ordinary request to the same site, `/sitemap.xml`, and asks the public GitHub API (`api.github.com`) when up to two repositories linked from the site were last updated; only the public repository name is sent. For up to two App Store apps linked from the site it asks Apple's public lookup API (`itunes.apple.com`) for the last update and rating count; only the app ID is sent. Trust & Commerce Readiness makes no requests: it never opens login or checkout pages. Domain & Identity sends only the domain name: to IANA's RDAP bootstrap list (`data.iana.org`), to the registry's public RDAP server for the top-level domain, and as two DNS TXT lookups (SPF and DMARC). Active probes, such as requesting well-known sensitive paths, run only with `--active`, which the user sets only for sites they own or are authorised to test.
